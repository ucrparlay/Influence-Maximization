#ifndef OUTSIDE_GRAPH_H
#define OUTSIDE_GRAPH_H
...
#include "outside_utilities.h"
using namespace std;
using namespace parlay;

namespace outside_graph {
...
static Graph read_pbbs(const char* const filename) {
...
static Graph read_binary(const char* const filename, bool enable_mmap = true) {
  Graph graph;
  if (enable_mmap) {
    struct stat sb;
...
static void make_symmetric(Graph& graph) {
...
static void make_directed(Graph& graph) {
...
static sequence<edge> StringToEdgeList(sequence<char> strings, long length){
  sequence<bool> flag(length);
  parallel_for(0, length, [&](size_t i) {
    if (is_space(strings[i])) {
      strings[i] = 0;
...
static Graph EdgeListToGraph(sequence<edge> edge_list, NodeId n){
...
static void read_knn(const char* const fileName, const char* const OutFileName,
              int k) {
  ifstream file(fileName, ifstream::in | ifstream::binary | ifstream::ate);
  if (!file.is_open()) {
    cerr << "Error: Cannot open file " << fileName << '\n';
...
static void read_txt(const char* const fileName, const char* const OutFileName, NodeId n){
  ifstream file(fileName, ifstream::in | ifstream::binary | ifstream::ate);
...
static Graph generate_synthetic_graph(size_t n, size_t k) {
  Graph graph;
  graph.n = n;
  graph.m = n * k;
  printf("n=%zu, m=%zu\n", graph.n, graph.m);
...
static Graph read_graph(char* filename, bool enable_mmap = true,
...
static Graph read_large_graph(char* filename) {
  ifstream ifs(filename);
  if (!ifs.is_open()) {
    cerr << "Error: Cannot open file " << filename << '\n';
    abort();
...
static Graph read_large_sym_graph(char* filename) {
  ifstream ifs(filename);
  if (!ifs.is_open()) {
    cerr << "Error: Cannot open file " << filename << '\n';
    abort();
...
static void AssignUniWeight(Graph& graph, float w){
...
static void AssignIndegreeWeightSym(Graph &graph){
  graph.W = sequence<float>(graph.m);
  parallel_for(0, graph.n, [&](size_t i){
...
static void AssignIndegreeWeightDir(Graph &graph){
  graph.in_W = sequence<float>(graph.m);
  graph.W = sequence<float>(graph.m);
...
static void AssignIndegreeWeight(Graph &graph){
  if (graph.symmetric){
