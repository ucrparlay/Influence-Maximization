...
def run_pmc(graph):
    ...
    file_out = f'{IM_DIR}/scripts/pmc_wic_results.txt'
    mem_out = f'{IM_DIR}/scripts/pmc_wic_mem.txt'
    subprocess.call(
        f'/usr/bin/time -v numactl -i all {pmc_command} 1>> {file_out} 2>> {mem_out}', shell=True)


if __name__ == '__main__':
    graphs = graph.get_directed_graphs()
    for g in graphs:
        run_pmc(g)
    # run_pmc('/data/graphs/bin/HT_5_sym.bin')
